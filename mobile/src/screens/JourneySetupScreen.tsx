...
export default function JourneySetupScreen({ navigation }: Props) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{t('journey.setup_title')}</Text>
      <TextInput
        style={styles.input}
        placeholder={t('journey.destination_placeholder')}
      ...
      <View style={styles.modeRow}>
        <TouchableOpacity style={[styles.modeButton, mode === 'interval' && styles.modeButtonActive]} onPress={() => setMode('interval')}>
          <Text style={[styles.modeText, mode === 'interval' && styles.modeTextActive]}>{t('journey.mode_interval')}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.modeButton, mode === 'scheduled' && styles.modeButtonActive]} onPress={() => setMode('scheduled')}>
          <Text style={[styles.modeText, mode === 'scheduled' && styles.modeTextActive]}>{t('journey.mode_scheduled')}</Text>
        </TouchableOpacity>
      </View>
      ...
      {mode === 'scheduled' ? (
        <>
          <Text style={styles.label}>{t('journey.deadline')}:</Text>
        ...
        <>
          <Text style={styles.label}>{t('journey.checkin_interval')}</Text>
          ...
          <View style={styles.row}>
            <Text style={styles.label}>{t('journey.geofence_warning')}</Text>
            <Switch 
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: '#6B7280', borderRadius: 8, padding: 14, fontSize: 16, marginBottom: 20, minHeight: 48 },
  label: { fontSize: 14, color: '#6B7280', marginBottom: 8 },
  modeRow: { flexDirection: 'row', marginBottom: 16, gap: 8 },
  modeButton: { flex: 1, borderWidth: 1, borderColor: '#6B21A8', borderRadius: 8, padding: 12, minHeight: 48, alignItems: 'center', justifyContent: 'center' },
  modeButtonActive: { backgroundColor: '#6B21A8' },
  modeText: { color: '#6B21A8', fontWeight: '600', textAlign: 'center' },
  modeTextActive: { color: '#fff' },
  timeButton: { borderWidth: 1, borderColor: '#6B21A8', borderRadius: 8, padding: 14, minHeight: 48, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  timeButtonText: { color: '#6B21A8', fontSize: 18, fontWeight: 'bold' },
  intervalRow: { flexDirection: 'row', marginBottom: 24 },
  intervalButton: { flex: 1, borderWidth: 1, borderColor: '#6B21A8', borderRadius: 8, padding: 12, marginRight: 8, minHeight: 48, alignItems: 'center', justifyContent: 'center' },
  ...
  button: { backgroundColor: '#D97706', borderRadius: 8, padding: 16, minHeight: 52, alignItems: 'center', justifyContent: 'center' },
