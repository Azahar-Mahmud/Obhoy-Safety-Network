...
router.post('/invite', authMiddleware, async (req, res) => {
  try {
    ...
    if (!rawPhone) return res.status(400).json({ error: 'Invalid phone number.' });

    const phoneE164 = normalizeToE164(rawPhone);
    const phoneLocal = phoneE164.replace(/^\+880/, '0');

    const me = await User.findById(req.userId);
    if (!me) return res.status(404).json({ error: 'User not found.' });
    if (me.phone === phoneE164 || me.phone === phoneLocal || me.phone === rawPhone) {
    ...
    // Match either +880... or 01... in User database
    const other = await User.findOne({
      $or: [{ phone: phoneE164 }, { phone: phoneLocal }, { phone: rawPhone }],
    });

    if (!other) {
      // Not an Obhoy user yet - send one SMS invite
      try {
        await sendSms(phoneE164, tSms(me.language, 'sms.family_invite', { name: me.phone }));
      } catch (err) {
    ...
    return res.json({ invited: true, smsSent: false });
  } catch (err) {
...
router.delete('/link/:userId', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Family unlink error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.post('/location', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({ published: true, sharingPaused: doc.sharingPaused });
  } catch (err) {
    console.error('Family location publish error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.post('/ping/:userId', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Family ping error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.patch('/sharing', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({ sharingPaused: paused });
  } catch (err) {
    console.error('Family sharing toggle error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.patch('/live', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json({ liveUntil: minutes > 0 ? new Date(Date.now() + minutes * 60000) : null });
  } catch (err) {
    console.error('Family live mode error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.get('/state', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Family state error:', err);
    return res.status(500).json({ error: err.message });
  }
});
...
router.get('/viewers', authMiddleware, async (req, res) => {
  try {
    ...
    console.error('Family viewers error:', err);
    return res.status(500).json({ error: err.message });
  }
});
